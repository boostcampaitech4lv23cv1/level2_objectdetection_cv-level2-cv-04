...
def main():
    ...
    # dataset load by fold_num
    cfg.data.train.ann_file = "/opt/level2_objectdetection_cv-level2-cv-04/folded_anns/"+ "train_"+f'{args.fold_num}' + '.json'
    # do not validation during train
    cfg.data.val.ann_file = "/opt/level2_objectdetection_cv-level2-cv-04/folded_anns/" + 'val_' + f'{args.fold_num}' + '.json'
    
    
    # do not testing during train 
    cfg.data.test.test_mode = False # test mode
    ## cfg.data.test.ann_file = cfg.data.test.img_prefix + 'test.json'
    ## cfg.data.test.pipeline[1]['img_scale'] = (1024,1024) # Resize test set
    
    cfg.data.samples_per_gpu = 1 # batch size
    
    # modify epochs, default=12
